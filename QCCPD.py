...
class change_point_detect(object):
    ...
    def __init__(self, dataframe, resample = True, write_dir = None, 
        ...
        self.years_list = list(set(df.index,year))
#------------------------------------------------------------------------------
    ...
    def get_change_points(self, n_trials = 1, keep_trial_results = False):
        ...
        trials_lst = []
        for year in years:
            print 'Getting change points for year {} - trial: '.format(str(year)),
            data_list = []
            ...
            results_df = pd.concat(data_list).reset_index(drop = True)
            trials_df, stats_df = self._cross_sample_stats_QC(results_df,
                                                              n_trials)
            stats_df['Year'] = year
